...
type WithLogicalOperators<T extends object> = T & {
  $and?: T[];
  $or?: T[];
};
...
>(transform: (data: Data) => NormalizedData): (data: Data) => NormalizedData {
  return (data: Data) => {
    const normalizedData = transform(
      data,
    ) as WithLogicalOperators<NormalizedData>;

    (["$and", "$or"] as const).forEach((key) => {
      if (normalizedData[key]) {
        normalizedData[key] = normalizedData[key]!.map(
          (item) => transform(item as unknown as Data) as NormalizedData,
        );
      }
    });
